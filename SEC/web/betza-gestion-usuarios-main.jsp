...
var obj = {
            ...
            showBottom: false,
            colModel: [
              	{ title: "Nombre", width: 100 }, 
                { title: "Apellido Paterno", width: 200 },
                { title: "apellido Materno", width: 150 },
                { title: "Escuela", width: 150 }
            ],
            dataModel: { data: arrayGrid}
        };
        
        $("<input type='button' value='Show Popup' />").
            appendTo($("#gridListaAlumno")).button().click(function (evt) {
        ...
        */
			...
			*/
			...
			/*------------------------------------------------------------------------------------
			-----Funcion para cambiar de minusculas a mayusculas en los campos del div buscar-----
			--------------------------------------------------------------------------------------*/
			function myFunction() {
    		//var x = document.getElementById("filtroNombre");
    		var a = document.getElementById('filtroNombre');
    		var b = document.getElementById('filtroApellidoP');
    		var c = document.getElementById('filtroApellidoM');
    		var d = document.getElementById('filtroEscuela');
			...
			//	---------------------------------------------------------------
			//   3.- Función que realiza busquedas
			//  ---------------------------------------------------------------
			/*
			 $(function () {
   		    function filter(evt, ui) {

            var $toolbar = $grid.find('divTblFiltros'),
                $value = $toolbar.find("#filtroBuscar"),
                value = $value.val(),
                nombre = $toolbar.find("#filtroNombre").val(),
                apellidoP = $toolbar.find("#filtroApellidoP").val(),
                apellidoM = $toolbar.find("#filtroApellidoM").val(),
                escuela = $toolbar.find("##filtroEscuela").val(),
                filterObject;

            if (nombre == "") {//search through all fields when no field selected.
                filterObject = [];
                var CM = $grid.pqGrid("getColModel");
                for (var i = 0, len = CM.length; i < len; i++) {
                    var nombre = CM[i].nombre;
                    filterObject.push({ nombre: nombre, apellidoP: apellidoP, apellidoM: apellidoM, escuela:escuela, value: value});
                }
            }
            else {//search through selected field.
                filterObject = [{nombre: nombre, apellidoP: apellidoP, apellidoM: apellidoM, escuela:escuela, value: value}];
            }
            $divTblFiltros("filter", {
                oper: 'replace',
                data: filterObject
            });
        }
		...
		<div id="tituloTab" >Gestión de Alumnos</div>
   		...
   		<div id="divTblFiltros" class="divTblFiltros" >
   			<div id = "filtroBuscar">
				...
				<input type="text" placeholder="Nombre" id="filtroNombre" maxlength="30" onblur="myFunction()" onkeypress="validarSimboloSuma(1);"></input> 
						
				<input type="text" placeholder="Apellido Paterno" id="filtroApellidoP" maxlength="20" onblur="myFunction()" onkeypress="validarSimboloSuma(1);"/> 
						
				<input type="text" placeholder="Apellido Materno" id="filtroApellidoM"   maxlength="50" onblur="myFunction()"></input>
				
				<input type="text" placeholder="Escuela" id="filtroEscuela"   maxlength="50" onblur="myFunction()"></input>
			</div>
			<div id="filtroBuscar">
					<div id="boton">
